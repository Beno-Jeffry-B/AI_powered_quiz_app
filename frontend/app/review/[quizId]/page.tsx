    ...
    return (
      <DashboardLayout>
        <div className="max-w-xl mx-auto py-20 text-center">
          <div className="bg-red-50 text-red-600 p-8 rounded-3xl border border-red-100 mb-6 font-bold">
            {error}
          </div>
          <button
        ...
        <div className="flex items-center justify-between mb-12">
          ...
          <div className="flex gap-4">
            <button
              ...
              className="px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700"
            >
              Retake Quiz
            </button>
            ...
            <button
              onClick={handleExit}
              className="px-6 py-3 bg-white border border-gray-200 text-gray-600 rounded-2xl font-bold hover:bg-gray-50"
            >
              Back to History
            </button>
          </div>
        ...
        <div className="space-y-10">
          {quiz.questions.map((q: any, index: number) => {
            ...
            return (
              <div
                key={q.id}
                className="bg-white rounded-[40px] shadow-xl border border-gray-100 p-8 relative"
              >
                {/* Status Bar */}
                <div
                  className={`absolute top-0 left-0 bottom-0 w-2 ${isCorrect ? "bg-green-500" : "bg-red-500"
                    }`}
                />
                ...
                <div className="grid gap-4">
                  {["A", "B", "C", "D"].map((label) => {
                    ...
                    // ✅ Correct selection
                    if (isUserSelection && q.is_correct) {
                      bgColor = "bg-green-50";
                    ...
                    // ❌ Wrong selection
                    else if (isUserSelection && !q.is_correct) {
                      bgColor = "bg-red-50";
                    ...
                    return (
                      <div
                        key={label}
                        className={`flex items-center gap-4 p-5 rounded-3xl border-2 ${bgColor} ${borderColor}`}
                      >
                        <span
